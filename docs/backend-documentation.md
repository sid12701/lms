...
| PUT | `/api/v1/internal/admin/lsps/{lspId}/status` | Disable or reactivate LSP | `SYSTEM_ADMIN` | `@PreAuthorize`, `@Valid` | `LspAdminController.updateStatus` | `status`, `reason`, `note` | LSP response | `lsp`, `api_client`, `lsp_audit_event` | Ops alerts | Disable: bumps `token_version`, deactivates clients, audit `LSP_DISABLED`, alert. Reactivate: LSP `ACTIVE` only. Same status → `400 STATUS_UNCHANGED`. |
| GET | `/api/v1/internal/admin/lsps/{lspId}/audit-events` | LSP status audit trail | `SYSTEM_ADMIN` | `@PreAuthorize` | `LspAdminController.listAuditEvents` | Path `lspId` | List of audit events | `lsp_audit_event` | None | Issue #63 |
