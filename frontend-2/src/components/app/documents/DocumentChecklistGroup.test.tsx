...
describe("DocumentChecklistGroup", () => {
  ...
  it("splits docs into required and optional lists", () => {
    ...
    const { container, getByText } = renderWithProviders(
      <DocumentChecklistGroup docs={docs} />,
    );
    const requiredSection = container.querySelector(
      '[data-slot="document-checklist-required"]',
    );
    const optionalSection = container.querySelector(
  ...
  it("renders an empty state for the optional section when there are none", () => {
    const { getByText } = renderWithProviders(
      <DocumentChecklistGroup
        docs={[makeDoc({ requiredForDisbursement: true })]}
      />,
    );
  ...
  it("renders DocumentUploadRow for PENDING docs and DocumentChecklistRow for UPLOADED docs", () => {
    ...
    expect(
      container.querySelectorAll('[data-slot="document-upload-row"]').length,
    ).toBe(1);
    expect(
      container.querySelectorAll('[data-slot="document-checklist-row"]').length,
    ).toBe(1);
  });

  it(
    "wires onUpload through to PENDING rows and forwards the doc",
    async () => {
  ...
  it(
    "wires onView/onDownload through to uploaded rows and forwards the doc",
    async () => {
      ...
      expect(onView).toHaveBeenCalledWith(
        expect.objectContaining({ id: uploadedDoc.id }),
      );
  ...
  it("propagates the compact prop to data-compact", () => {
    const { container } = renderWithProviders(
      <DocumentChecklistGroup docs={[]} compact />,
    );
