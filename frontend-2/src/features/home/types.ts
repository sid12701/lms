 ...
 *
 * `mocks/api/home.ts` returns these shapes, `features/home/components/*`
 * consume them, and `features/home/page.tsx` wires the two together via
 * a TanStack Query hook. Keeping the contract in one file lets the three
 * concerns ship in parallel without coordination drift.
 *
 * Types intentionally use plain TS (not Zod-inferred) — these are
 * presentation projections, not domain entities. The mock API is free to
 * change its internal shape as long as it serialises to these.
 */
 ...
 * `bucket`. Computed from `db.installments` in the mock API.
 */
