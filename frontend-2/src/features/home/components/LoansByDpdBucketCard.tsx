...
export const LoansByDpdBucketCard = forwardRef<HTMLDivElement, LoansByDpdBucketCardProps>(
  function LoansByDpdBucketCard({ buckets, isLoading = false, className }, ref) {
    ...
    return (
      <Card
        ref={ref}
        data-slot="loans-by-dpd"
        className={cn(className)}
        aria-labelledby={headingId}
        aria-describedby={summaryId}
      >
        <CardHeader>
          <CardTitle id={headingId}>Loans by DPD bucket</CardTitle>
          <CardDescription>
            Distribution of overdue loans across delinquency buckets.
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          {total === 0 ? (
            <EmptyState
              icon={BarChart3}
              title="No loans in DPD buckets"
              description="When repayment schedules are available, loans will appear here grouped by DPD bucket."
            />
          ) : (
            <div
