...
It is meant to be readable in 10 minutes by anyone picking up the project mid-flight. It is **not** authoritative — `docs/frontend-implementation-plan.md` is. When the two disagree, prefer the plan.

---

## 1. What this repo is

The **frontend (UI) implementation** of the Bhawana Capital Loan Management System (LMS). The backend lives in a separate repo and is **not built, run, or mocked at the network layer here** — all data is mocked in-app via `src/mocks/api/*`. No real API calls, no auth handshakes, no network requests to a server.
