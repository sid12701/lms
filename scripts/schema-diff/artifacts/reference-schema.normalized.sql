...
$$;
CREATE FUNCTION public.maintain_loan_event_partitions(lead_months integer, retention_days integer) RETURNS TABLE(action text, partition_name text)
    LANGUAGE plpgsql
    AS $$
DECLARE
...
END
$$;
CREATE FUNCTION public.reject_disbursement_observation_mutation() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
END
$$;
CREATE FUNCTION public.reject_loan_event_mutation() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE TABLE public.api_client (
    ...
    credentials_invalidated_at timestamp with time zone
);
ALTER TABLE ONLY public.api_client FORCE ROW LEVEL SECURITY;
...
CREATE TABLE public.auth_session (
    id uuid NOT NULL,
    user_id uuid NOT NULL,
    policy_epoch character varying(128) NOT NULL,
...
ALTER TABLE ONLY public.borrower FORCE ROW LEVEL SECURITY;
...
ALTER TABLE ONLY public.borrower_bank_details_update_audit FORCE ROW LEVEL SECURITY;
CREATE TABLE public.borrower_lsp_access (
    borrower_id uuid NOT NULL,
    lsp_id uuid NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL
);
ALTER TABLE ONLY public.borrower_lsp_access FORCE ROW LEVEL SECURITY;
CREATE TABLE public.borrower_lsp_relationship (
    id uuid NOT NULL,
    borrower_id uuid NOT NULL,
    lsp_id uuid NOT NULL,
    first_sourced_at timestamp with time zone DEFAULT now() NOT NULL,
...
ALTER TABLE ONLY public.borrower_lsp_relationship FORCE ROW LEVEL SECURITY;
...
CREATE TABLE public.disbursement_intent (
    id uuid NOT NULL,
    loan_account_id uuid NOT NULL,
    tran_ref_no character varying(64) NOT NULL,
    amount numeric(19,2) NOT NULL,
    payment_mode character varying(16) NOT NULL,
    beneficiary_name character varying(255) NOT NULL,
    beneficiary_account_number character varying(64) NOT NULL,
    beneficiary_ifsc character varying(16) NOT NULL,
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    schedule_hash character varying(64),
    CONSTRAINT disbursement_intent_amount_check CHECK ((amount > (0)::numeric))
);
ALTER TABLE ONLY public.disbursement_intent FORCE ROW LEVEL SECURITY;
CREATE TABLE public.disbursement_observation (
    id uuid NOT NULL,
    loan_account_id uuid NOT NULL,
    intent_id uuid,
    ...
    query_resolved boolean DEFAULT true NOT NULL,
    is_duplicate boolean DEFAULT false NOT NULL,
    provider_name character varying(64) NOT NULL,
    provider_request_id character varying(128),
    act_code character varying(16),
    bank_rrn character varying(32),
    decline_kind character varying(16),
    beneficiary_ifsc character varying(16),
    beneficiary_account_number character varying(64),
    payment_mode character varying(16),
    request_payload_json jsonb NOT NULL,
    response_payload_json jsonb NOT NULL,
    correlation_id character varying(128),
    provenance character varying(16) DEFAULT 'LIVE'::character varying NOT NULL,
    created_by character varying(255) NOT NULL,
    observed_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT disbursement_observation_disposition_check CHECK (((disposition)::text = ANY ((ARRAY['SUCCESS'::character varying, 'FAILED'::character varying, 'PENDING'::character varying])::text[]))),
    CONSTRAINT disbursement_observation_kind_check CHECK (((kind)::text = ANY ((ARRAY['INITIATE'::character varying, 'POLL'::character varying, 'LEGACY'::character varying])::text[]))),
    CONSTRAINT disbursement_observation_provenance_check CHECK (((provenance)::text = ANY ((ARRAY['LIVE'::character varying, 'LEGACY'::character varying])::text[])))
);
ALTER TABLE ONLY public.disbursement_observation FORCE ROW LEVEL SECURITY;
...
CREATE TABLE public.disbursement_reconciliation_queue (
    loan_account_id uuid NOT NULL,
    intent_id uuid,
    tran_ref_no character varying(64),
    ...
    last_observation_at timestamp with time zone DEFAULT now() NOT NULL,
    owner character varying(128),
    escalated boolean DEFAULT false NOT NULL,
    details text,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT disbursement_reconciliation_queue_reason_check CHECK (((reason)::text = ANY ((ARRAY['UNKNOWN'::character varying, 'REQUESTED'::character varying, 'PARKED'::character varying, 'LEGACY_MISMATCH'::character varying, 'STRANDED_TERMINAL'::character varying, 'CONFLICTING_EVIDENCE'::character varying])::text[])))
);
ALTER TABLE ONLY public.disbursement_reconciliation_queue FORCE ROW LEVEL SECURITY;
...
ALTER TABLE ONLY public.loan_account FORCE ROW LEVEL SECURITY;
...
ALTER TABLE ONLY public.loan_application_assignment_event FORCE ROW LEVEL SECURITY;
...
ALTER TABLE ONLY public.loan_application_audit_event FORCE ROW LEVEL SECURITY;
...
ALTER TABLE ONLY public.loan_application_document_access_audit FORCE ROW LEVEL SECURITY;
CREATE TABLE public.loan_application_document_access_audit_type (
    audit_id uuid NOT NULL,
    document_type character varying(64) NOT NULL
);
ALTER TABLE ONLY public.loan_application_document_access_audit_type FORCE ROW LEVEL SECURITY;
...
ALTER TABLE ONLY public.loan_application_intake_audit FORCE ROW LEVEL SECURITY;
...
ALTER TABLE ONLY public.loan_application_pii_reveal_audit FORCE ROW LEVEL SECURITY;
...
ALTER TABLE ONLY public.loan_application_status_transition FORCE ROW LEVEL SECURITY;
...
ALTER TABLE ONLY public.loan_payment_transaction FORCE ROW LEVEL SECURITY;
...
ALTER TABLE ONLY public.loan_product_lsp_mapping FORCE ROW LEVEL SECURITY;
...
ALTER TABLE ONLY public.lsp_api_idempotency_record FORCE ROW LEVEL SECURITY;
...
CREATE TABLE public.refresh_token (
    ...
    revoked_at timestamp with time zone,
    issued_tv bigint,
    issued_pwdv_millis bigint,
    issued_policy_epoch character varying(128),
...
ALTER TABLE ONLY public.borrower_lsp_relationship
    ADD CONSTRAINT borrower_lsp_relationship_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.disbursement_intent
    ADD CONSTRAINT disbursement_intent_pkey PRIMARY KEY (id);
ALTER TABLE ONLY public.disbursement_observation
    ADD CONSTRAINT disbursement_observation_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.disbursement_reconciliation_queue
    ADD CONSTRAINT disbursement_reconciliation_queue_pkey PRIMARY KEY (loan_account_id);
...
CREATE INDEX idx_auth_session_user_revoked ON public.auth_session USING btree (user_id, revoked);
...
CREATE INDEX idx_borrower_lsp_relationship_borrower ON public.borrower_lsp_relationship USING btree (borrower_id);
CREATE INDEX idx_borrower_lsp_relationship_lsp ON public.borrower_lsp_relationship USING btree (lsp_id);
...
CREATE INDEX idx_disbursement_intent_claimable ON public.disbursement_intent USING btree (state, lease_expires_at) WHERE ((state)::text = 'CREATED'::text);
CREATE INDEX idx_disbursement_observation_account_time ON public.disbursement_observation USING btree (loan_account_id, observed_at DESC);
CREATE INDEX idx_disbursement_observation_disposition ON public.disbursement_observation USING btree (disposition, query_resolved) WHERE (query_resolved AND (NOT is_duplicate));
CREATE INDEX idx_disbursement_observation_intent ON public.disbursement_observation USING btree (intent_id, observed_at DESC) WHERE (intent_id IS NOT NULL);
CREATE INDEX idx_disbursement_observation_tran_ref ON public.disbursement_observation USING btree (tran_ref_no, observed_at DESC);
...
CREATE INDEX idx_disbursement_recon_next_poll ON public.disbursement_reconciliation_queue USING btree (next_poll_at);
CREATE INDEX idx_disbursement_recon_reason ON public.disbursement_reconciliation_queue USING btree (reason, first_seen_at);
...
CREATE INDEX idx_lsp_api_idempotency_pending_lease ON public.lsp_api_idempotency_record USING btree (lease_expires_at) WHERE (response_body = '{"__idempotencyPending":true}'::text);
...
CREATE INDEX idx_refresh_token_family ON public.refresh_token USING btree (family_id);
CREATE INDEX idx_refresh_token_family_revoked ON public.refresh_token USING btree (family_id, revoked);
...
ALTER TABLE ONLY public.borrower_lsp_relationship
    ADD CONSTRAINT borrower_lsp_relationship_borrower_id_fkey FOREIGN KEY (borrower_id) REFERENCES public.borrower(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.borrower_lsp_relationship
    ADD CONSTRAINT borrower_lsp_relationship_lsp_id_fkey FOREIGN KEY (lsp_id) REFERENCES public.lsp(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.borrower_pii_reveal_audit
...
ALTER TABLE ONLY public.disbursement_intent
    ADD CONSTRAINT disbursement_intent_loan_account_id_fkey FOREIGN KEY (loan_account_id) REFERENCES public.loan_account(id);
ALTER TABLE ONLY public.disbursement_observation
    ADD CONSTRAINT disbursement_observation_intent_id_fkey FOREIGN KEY (intent_id) REFERENCES public.disbursement_intent(id);
ALTER TABLE ONLY public.disbursement_observation
    ADD CONSTRAINT disbursement_observation_loan_account_id_fkey FOREIGN KEY (loan_account_id) REFERENCES public.loan_account(id);
ALTER TABLE ONLY public.disbursement_outcome_audit
...
ALTER TABLE ONLY public.refresh_token
    ADD CONSTRAINT refresh_token_family_id_fkey FOREIGN KEY (family_id) REFERENCES public.auth_session(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.report_access_audit
...
ALTER TABLE public.borrower_bank_details_update_audit ENABLE ROW LEVEL SECURITY;
CREATE POLICY borrower_bank_details_update_audit_tenant_policy ON public.borrower_bank_details_update_audit TO lms_tenant_app USING ((lsp_id = public.app_current_lsp_id())) WITH CHECK ((lsp_id = public.app_current_lsp_id()));
...
ALTER TABLE public.borrower_lsp_relationship ENABLE ROW LEVEL SECURITY;
CREATE POLICY borrower_lsp_relationship_tenant_policy ON public.borrower_lsp_relationship TO lms_tenant_app USING ((lsp_id = public.app_current_lsp_id())) WITH CHECK ((lsp_id = public.app_current_lsp_id()));
...
CREATE POLICY borrower_tenant_update_policy ON public.borrower FOR UPDATE TO lms_tenant_app USING ((EXISTS ( SELECT 1
  ...
  WHERE ((access.borrower_id = borrower.id) AND (access.lsp_id = public.app_current_lsp_id()))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.borrower_lsp_access access
  WHERE ((access.borrower_id = borrower.id) AND (access.lsp_id = public.app_current_lsp_id())))));
ALTER TABLE public.disbursement_intent ENABLE ROW LEVEL SECURITY;
CREATE POLICY disbursement_intent_tenant_policy ON public.disbursement_intent TO lms_tenant_app USING (public.tenant_owns_loan_account(loan_account_id)) WITH CHECK (public.tenant_owns_loan_account(loan_account_id));
ALTER TABLE public.disbursement_observation ENABLE ROW LEVEL SECURITY;
CREATE POLICY disbursement_observation_tenant_policy ON public.disbursement_observation TO lms_tenant_app USING (public.tenant_owns_loan_account(loan_account_id)) WITH CHECK (public.tenant_owns_loan_account(loan_account_id));
ALTER TABLE public.disbursement_reconciliation_queue ENABLE ROW LEVEL SECURITY;
CREATE POLICY disbursement_reconciliation_queue_tenant_policy ON public.disbursement_reconciliation_queue TO lms_tenant_app USING (public.tenant_owns_loan_account(loan_account_id)) WITH CHECK (public.tenant_owns_loan_account(loan_account_id));
