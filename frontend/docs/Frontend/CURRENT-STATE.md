...
> **2026-07-13 update (Spec S1 / CI-01):** Frontend release baseline restored. `npm run lint`, `format:check`, `typecheck`, `build`, and `npm run test` are green (`754` tests / `123` files). Changes: removed dead `AuditTable` props; audit cursor pagination via `accumulateAuditRowsReducer` (no synchronous `setState` in effects); loan-detail servicing extracted to `LoanServicingPanel` + `useMyLoanServicing` (TanStack Query); `DocumentsSection` re-fetches when `docLabels` load; fixture builders `makeMyLoanDetail` / `makeCreateUserInput`; `scripts/verify.mjs` runs all gates with combined failure reporting; Vitest `testTimeout` 15s; OpenAPI type generation pipes through Prettier. Residual: ~20 `act(...)` warnings in axe specs.

> **2026-07-15 update (Specs S7–S12):** Same-origin credential gate (`http-client`); access token memory-only (`session-storage`); loan-account status vocab aligned to backend; disbursement money preview + reference APIs in confirm dialog; E2E env-only login + `globalSetup` fixtures (`docs/e2e.md`); Home/Audit `@canary` + bootstrap-sync. See `docs/implementation-log.md`.
...
| BR-11 | LSP-provided schedule end-to-end validation | FE mock: `schemas/lsp-provided-schedule.ts` + mock router. **Server (authoritative, Spec S20 2026-07-15):** principal + date/interest checks — `docs/partner-schedule-validation.md` |
...
- ⚠️ BR-8 per-client IP allow-list editor on API client create + rotate forms — **superseded**: allow-lists are now managed per LSP (UI/API surfaces), not per client
