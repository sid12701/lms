...
public interface AdminApiIdempotencyRecordRepository extends JpaRepository<AdminApiIdempotencyRecord, UUID> {

    Optional<AdminApiIdempotencyRecord> findByOperationKeyAndIdempotencyKey(
            String operationKey,
    ...
    @Query("select min(record.createdAt) from AdminApiIdempotencyRecord record where record.responseBody = :responseBody")
    ...
    @Transactional
    @Query("""
            DELETE FROM AdminApiIdempotencyRecord record
            WHERE record.id = :id
              AND record.attempt = :attempt
              AND record.leaseOwner = :leaseOwner
              AND record.responseBody = :pendingBody
