...
/**
 * Loan creation stays on the tenant datasource (row-level security on), so
 * {@link LoanApplicationOnboardingService} re-asserts tenant ownership in code: an LSP-authenticated
 * create must be written for the LSP the caller authenticated as.
 */
...
class LoanApplicationOnboardingServiceLspGuardTest {
    ...
    private void runScopePreservingInline() {
        when(scopePreservingTransactionExecutor.call(any()))
